...
LibertyBear APIs are derived from those in Greasemonkey v3, and most of them work the same way, `GM.*` [Greasemonkey v4-compatible](https://wiki.greasespot.net/Greasemonkey_Manual:API) aliases were added in VM2.12.0.

## GM_*

`GM_*` are a group of special APIs provided by LibertyBear.
...
- `scriptHandler` *string*
    ...
    The name of userscript manager, which should be the string `LibertyBear`.

- `version` *string*

    Version of LibertyBear.

- `platform` *object* *(since VM2.12.4)*

    Unlike `navigator.userAgent`, which can be overriden by other extensions/userscripts or by devtools in device-emulation mode, `GM_info.platform` is more reliable as the data is obtained in the background page of LibertyBear using a specialized extension API (browser.runtime.getPlatformInfo and getBrowserInfo).
...
Older versions of LibertyBear (prior to 2.12.0) returned an imitation of Promise,
...
Registers a command in LibertyBear popup menu.
...
Unregisters a command which has been registered to LibertyBear popup menu.
